...
class Bert4CNNerFunction(BaseFunction):
    def __init__(self):
        super().__init__()
        ...
        for i in range(batch_size):
            ...
            for j in range(max_len):
                if label_masks[i][j].item() == 1:
                    ...
                    valid_len[i] = seq_len - 1
                    break
        metrics.evaluate(prediction, label_ids, valid_len)


class Bert4CNNer(Bert4CNNerFunction, nn.Module):
    def __init__(
            self,
            vocabulary,
            embedding_size=768,
...
class Bert4CNNerParallel(nn.DataParallel, Bert4CNNerFunction):
